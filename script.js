...
if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('sw.js').catch(err => console.log('SW falhou')));
}
...
// --- LOGIN MULTIUSUÁRIO (CPF AUTOMÁTICO) ---
...
window.handleSaveAlarm = function() {
    ...
    if(!desc || !date || !time) return showToast("Preencha todos os campos do alarme!", true);
...
window.showFeiraScreen = function() { window.showScreen('feira-screen'); renderFeira(); };
window.closeFeiraScreen = function() { window.showScreen('main-screen'); };
...
window.handleSaveFeiraItem = function() {
    ...
    const save = () => {
        ...
        saveDB(); renderFeira(); window.closeModals(); showToast("Item Salvo!");
    };
...
function renderFeira() {
    ...
    db.feiraItems.forEach(i => {
        total += (i.val * i.qtd);
        list.innerHTML += `<div class="expense-item" style="border-left-color: var(--success);"><div class="expense-info"><strong>${i.name}</strong><small>${i.qtd}x R$ ${i.val.toFixed(2)}</small></div><div class="action-btns"><button onclick="deleteFeiraItem(${i.id})" style="color:var(--danger)">🗑</button></div></div>`;
    });
