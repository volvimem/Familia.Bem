...
function saveDB() {
    if (currentUid) {
        set(ref(dbFirebase, 'couples/' + currentUid), db);
    }
}

// Escutar alterações do Firebase em tempo real (Sincroniza os dois celulares na hora)
function listenToCoupleData() {
    onValue(ref(dbFirebase, 'couples/' + currentUid), (snapshot) => {
        const data = snapshot.val();
        if (data) {
            db = data;
            // Garante que arrays existam mesmo se estiverem vazios no Firebase
            ...
            // Cria a base inicial caso seja o primeiro acesso da família
            db = {
                categories: ['Alimentação', 'Contas da Casa', 'Lazer', 'Viagem', 'Mercado'],
                entries: [], feiraItems: [], notificationsLog: []
            };
            saveDB();
        }
...
window.addEventListener('beforeinstallprompt', (e) => {
    e.preventDefault(); deferredPrompt = e; document.getElementById('btn-install').style.display = 'inline-block';
});
...
// --- LOGIN MULTIUSUÁRIO (CPF AUTOMÁTICO PARA EMAIL) ---
window.handleRegister = async function() {
    const rawCpf = document.getElementById('reg-cpf').value;
    const cpf = rawCpf.replace(/\D/g, ''); // Extrai apenas os números
    const pass = document.getElementById('reg-pass').value;

    if(!cpf || !pass || pass.length < 6) return showToast("Digite o CPF e uma senha de 6 dígitos!", true);
    
    // Converte discretamente o CPF para um padrão de email aceito pelo Firebase
    const email = `${cpf}@familiabem.com`;

    try {
        await createUserWithEmailAndPassword(auth, email, pass);
        showToast("Família registrada com sucesso! Faça o Login.");
...
window.attemptLogin = async function() {
    const rawCpf = document.getElementById('login-cpf').value;
    ...
    try {
        ...
        listenToCoupleData(); // Começa a ouvir o DB em tempo real

        if ("Notification" in window) {
            Notification.requestPermission().then(permission => { if (permission === "granted") checkTodayInstallments(); });
        }
    } catch(error) {
...
// --- SISTEMA DE AVISOS E NAVEGAÇÃO ---
function sendNotification(title, body) {
    if ("Notification" in window && Notification.permission === "granted") new Notification(title, { body: body, icon: 'icon-512.png' });
}
...
function logNotification(text) {
    ...
    if(db.notificationsLog.length > 50) db.notificationsLog.pop();
    saveDB();
}
...
function showToast(msg, isError = false) {
    ...
    t.className = "show"; setTimeout(() => { t.className = t.className.replace("show", ""); }, 2900);
}
...
function showConfirmModal(title, msg, onConfirm) {
    ...
    document.getElementById('btn-confirm-action').onclick = () => { if(pendingConfirmAction) pendingConfirmAction(); window.closeModals(); };
...
window.confirmAddCategory = function() {
    ...
    if (newCat) { db.categories.push(newCat); saveDB(); showToast("Categoria Adicionada!"); window.closeModals(); }
};

window.openAddModal = function() {
    document.getElementById('edit-id').value = ''; document.getElementById('form-title').innerText = "Nova Despesa";
    ...
    document.getElementById('parcelas-container').style.display = 'block'; document.getElementById('modal-add').classList.add('active');
};
    ...
    const saveAction = () => {
        if (editId) {
            ...
            if (currentView === 'home') {
                ...
                sendNotification("Nova Despesa da Casa", msg); logNotification(msg);
            }
        }
...
window.editEntry = function(id) {
    ...
    document.getElementById('edit-id').value = e.id; document.getElementById('form-title').innerText = "Editar Registro";
    document.getElementById('exp-desc').value = e.desc; document.getElementById('exp-val').value = e.val;
    document.getElementById('exp-cat').value = e.category; document.getElementById('exp-date').value = e.date;
    document.getElementById('exp-split').value = e.split; document.getElementById('parcelas-container').style.display = 'none';
...
window.deleteEntry = function(id) {
    showConfirmModal("Excluir", "Tem certeza que deseja apagar este registro?", () => {
        ...
        saveDB(); showToast("Removido!");
    });
};

// --- ALARMES E FEIRA ---
window.openAlarmModal = function() { document.getElementById('modal-alarm').classList.add('active'); };
window.handleSaveAlarm = function() {
    const desc = document.getElementById('alarm-desc').value; const date = document.getElementById('alarm-date').value; const time = document.getElementById('alarm-time').value;
    db.entries.push({ id: Date.now(), isAlarm: true, desc: "⏰ " + desc, date, time, owner: currentUser, type: currentView });
    saveDB(); window.closeModals(); showToast("Alarme Agendado!");
};

window.showFeiraScreen = function() { showScreen('feira-screen'); renderFeira(); };
window.closeFeiraScreen = function() { showScreen('main-screen'); };
...
window.handleSaveFeiraItem = function() {
    const id = document.getElementById('feira-edit-id').value; const name = document.getElementById('feira-item-name').value;
    const val = parseFloat(document.getElementById('feira-item-val').value); const qtd = parseFloat(document.getElementById('feira-item-qtd').value);
    const save = () => {
        if(id) { const idx = db.feiraItems.findIndex(i => i.id == id); db.feiraItems[idx] = { id, name, val, qtd }; } else { db.feiraItems.push({ id: Date.now(), name, val, qtd }); }
        saveDB(); renderFeira(); window.closeModals();
    };
...
window.deleteFeiraItem = function(id) { showConfirmModal("Remover", "Tirar item do carrinho?", () => { db.feiraItems = db.feiraItems.filter(i => i.id !== id); saveDB(); renderFeira(); }); };
window.clearFeira = function() { showConfirmModal("Limpar Tudo", "Deseja esvaziar o carrinho?", () => { db.feiraItems = []; saveDB(); renderFeira(); }); };
...
function renderAll() {
    ...
    homeMonthEntries.forEach(e => {
        if (e.split === 50) { totalM += (e.val/2); totalE += (e.val/2); }
            ...
            if (e.split === -100) { totalE += e.val; debtM += (e.val/2); } else { totalM += e.val; if(e.split === 100) debtE += (e.val/2); }
        }
        else if (e.owner === 'esposa') {
            if (e.split === -100) { totalM += e.val; debtE += (e.val/2); } else { totalE += e.val; if(e.split === 100) debtM += (e.val/2); }
        }
    });
    ...
    const container = document.getElementById('list-container'); container.innerHTML = '<h4>Lançamentos do dia</h4>';
...
function drawChart(data) {
    const canvas = document.getElementById('expense-chart'); const ctx = canvas.getContext('2d'); ctx.clearRect(0,0,160,160);
...
setInterval(() => {
    ...
    db.entries.forEach(e => { if(e.isAlarm && e.date === d && e.time === t && !e.triggered) { sendNotification("⏰ Lembrete!", e.desc); e.triggered = true; saveDB(); } });
}, 60000);
